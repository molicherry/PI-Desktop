...
| D407 | 插件的宿主 turn 结束事件 | **`session:turnEnded` 是宿主事件，载荷为 `{ sessionId, turnId, reason }`（`completed` / `aborted` / `error`），对 `session.beginTurn` 真正开始的每个 turn，在 turn 拆除结束时、持久化的 `session.endTurn` 尝试之后广播一次。携带的 `turnId` 是终止运行时事件本身所标识的身份，而不是恰好活动的那个 turn；`ToolExecContext.turnId` 也会填充同一值。没有 ack，也没有重放：存活且已订阅的插件只收到一次；与崩溃、重载或宿主退出竞态的投递不作保证；收到该事件也不代表该 turn 的所有在途工具都已退出，因此清理必须按 `turnId` 串行化或限定作用域。不需要新权限，0.14.7 之前的宿主从不发出该事件。见 ADR 0233。** | 驱动 GUI 的插件此前只能用空闲计时器猜测 turn 是否结束，而计时器会在 turn 中途触发、在 turn 结束后再次触发。宿主拥有的「每个 turn 一次」终止事件加上明确的 turn 身份，让插件可以精确结算一次，工具上下文中的同一身份还能用来关联迟到的工具结果。 |
...
## 2026-09-12 —— 插件的宿主 turn 结束事件（D407）

- `session:turnEnded` 发送到插件进程和插件面板页面，载荷为
  `{ sessionId, turnId, reason }`，其中 `reason` 为 `completed`、`aborted` 或
  `error`；对 `session.beginTurn` 真正开始的每个 turn（用户提交、已批准的计划执行、
  定时运行）发送一次。
- 该通知在 turn 拆除结束时、持久化的 `session.endTurn` 尝试之后发出，携带终止
  运行时事件的 `turnId` 而不是恰好活动的 turn。`ToolExecContext.turnId` 会填充
  同一身份。
- 该事件不需要权限，没有 ack 也没有重放；跨插件崩溃、重载或宿主退出的投递不作
  保证，也不代表该 turn 的所有在途工具都已退出。0.14.7 之前的宿主从不发出它。
- 决策 D407 由 ADR 0233 记录。
