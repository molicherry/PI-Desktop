...
## 2026-09-12 — Host turn-end event for plugins (D407)

- `session:turnEnded` is delivered to plugin processes and plugin panel pages
  with `{ sessionId, turnId, reason }`, where `reason` is `completed`,
  `aborted`, or `error`, once per turn actually started by `session.beginTurn`
  (a user submission, an approved plan execution, or a scheduled run).
- The announcement runs at the end of turn teardown, after the durable
  ...
  event rather than whichever turn is active. `ToolExecContext.turnId` is
  populated with the same identity.
- The event needs no permission, has no ack and no replay, is not guaranteed
  ...
  in-flight tool of the turn has exited. Hosts before 0.14.7 never emit it.
- Decision D407 is recorded by ADR 0233.
